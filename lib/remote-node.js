...
var RemoteNode = {
  Base: {
    ...
    _remote_setValue: function(value) {
      ...
      if ((typeof value == 'object') && value.t && (value.t == 'fileinput')) {
        var d = Util.Promise.defer();
        CTS.engine.server.uploadFiles(value.o).then(
          function(urls) {
            transform.newvalue = urls.join(',');
            self.tree._remote_sendTransform(transform).then(
              function(x) {
                d.resolve(x);
              }
            ).done();
